...
@WebServlet("/relatorios/romaneio-carga")
public class RelatorioRomaneioCargaServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (usuarioLogado.isAdmin() && !deveGerarRelatorio(request)) {
            ...
            request.getRequestDispatcher("/WEB-INF/jsp/relatorio/filtroRomaneioCarga.jsp").forward(request, response);
            return;
        }
        ...
        try {
            ...
            try (InputStream jrxmlStream = getServletContext().getResourceAsStream(
                    "/reports/romaneio_carga.jrxml")) {
                if (jrxmlStream == null) {
                    throw new ServletException("Arquivo do relatorio nao encontrado em /reports/romaneio_carga.jrxml");
                }
            ...
            response.setHeader(
                    "Content-Disposition",
                    "inline; filename=relatorio_romaneio_carga.pdf"
            );
            ...
            e.printStackTrace();
            throw new ServletException("Erro ao gerar relatorio de romaneio de carga.", e);
