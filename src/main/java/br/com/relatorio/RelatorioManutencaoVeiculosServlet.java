...
@WebServlet("/relatorios/manutencoes-veiculos")
public class RelatorioManutencaoVeiculosServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!deveGerarRelatorio(request)) {
            if (usuarioLogado.isAdmin()) {
                request.setAttribute("clientes", clienteDAO.listarTodos());
            }
            request.setAttribute("statusManutencaoOptions", StatusManutencao.values());
            request.getRequestDispatcher("/WEB-INF/jsp/relatorio/filtroManutencaoVeiculos.jsp")
        ...
        String statusFiltro = resolverStatusFiltro(request);
        ...
        try {
            ...
            parametros.put("STATUS_MANUTENCAO", statusFiltro);

            JasperPrint jasperPrint;

            try (InputStream jrxmlStream = getServletContext().getResourceAsStream(
                    "/reports/manutencoes_veiculos.jrxml")) {
                if (jrxmlStream == null) {
                    throw new ServletException(
                            "Arquivo do relatorio nao encontrado em /reports/manutencoes_veiculos.jrxml");
                }
            ...
            response.setHeader(
                    "Content-Disposition",
                    "inline; filename=relatorio_manutencoes_veiculos.pdf"
            );
            ...
            e.printStackTrace();
            throw new ServletException("Erro ao gerar relatorio de manutencoes de veiculos.", e);
    ...
    private Integer resolverClienteIdFiltro(HttpServletRequest request, Usuario usuarioLogado) {
        ...
        try {
            return Integer.valueOf(clienteIdParam.trim());
        } catch (NumberFormatException e) {
            return null;
        }
    }

    private String resolverStatusFiltro(HttpServletRequest request) {
        ...
        try {
            return StatusManutencao.valueOf(statusParam.trim()).name();
