...
public class FreteDAO extends ConnectionFactory {
    ...
    public String gerarProximoNumeroFrete(int ano) {
        String sql = "SELECT COALESCE(MAX(CAST(SUBSTRING(numero_frete FROM 10 FOR 5) AS INTEGER)), 0) "
                + "FROM frete WHERE numero_frete LIKE ?";
        ...
        try (Connection conn = getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    int proximoSequencial = rs.getInt(1) + 1;
        ...
        return String.format("FRT-%d-%05d", ano, 1);
    }
